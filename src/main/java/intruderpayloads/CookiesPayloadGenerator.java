...
public class CookiesPayloadGenerator implements PayloadGenerator {

    private final List<String> payloads;
    private int payloadIndex = 0;

    public CookiesPayloadGenerator() throws IOException {
        payloads = new ArrayList<>();

        // Load the index file that lists all payload files
        InputStream indexStream = getClass().getClassLoader().getResourceAsStream("Cookies-index.txt");
        if (indexStream == null) {
            throw new FileNotFoundException("Cookies-index.txt not found in resources.");
        }
