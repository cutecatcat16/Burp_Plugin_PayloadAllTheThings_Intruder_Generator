...
public class OpenRedirectPayloadGenerator implements PayloadGenerator {

    private final List<String> payloads;
    private int payloadIndex = 0;

    public OpenRedirectPayloadGenerator() throws IOException {
        payloads = new ArrayList<>();

        // Load the index file that lists all payload files
        InputStream indexStream = getClass().getClassLoader().getResourceAsStream("OpenRedirect-index.txt");
        if (indexStream == null) {
            throw new FileNotFoundException("OpenRedirect-index.txt not found in resources.");
        }
