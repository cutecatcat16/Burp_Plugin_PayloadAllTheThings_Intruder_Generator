...
public class NoSQLPayloadGenerator implements PayloadGenerator {

    private final List<String> payloads;
    private int payloadIndex = 0;

    public NoSQLPayloadGenerator() throws IOException {
        payloads = new ArrayList<>();

        // Load the index file that lists all payload files
        InputStream indexStream = getClass().getClassLoader().getResourceAsStream("NoSQL-index.txt");
        if (indexStream == null) {
            throw new FileNotFoundException("NoSQL-index.txt not found in resources.");
        }
