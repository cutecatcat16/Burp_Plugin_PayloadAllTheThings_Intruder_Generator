...
public class InjectionsGenerator implements PayloadGenerator {

    private final List<String> payloads;
    private int payloadIndex = 0;

    public InjectionsGenerator() throws IOException {
        payloads = new ArrayList<>();

        // Load the index file that lists all payload files
        InputStream indexStream = getClass().getClassLoader().getResourceAsStream("Injection-index.txt");
        if (indexStream == null) {
            throw new FileNotFoundException("Injection-index.txt not found in resources.");
        }
