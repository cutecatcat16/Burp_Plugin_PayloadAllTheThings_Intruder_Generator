...
public class SSRFPayloadGenerator implements PayloadGenerator {

    private final List<String> payloads;
    private int payloadIndex = 0;

    public SSRFPayloadGenerator() throws IOException {
        payloads = new ArrayList<>();

        // Load the index file that lists all payload files
        InputStream indexStream = getClass().getClassLoader().getResourceAsStream("ssrf-index.txt");
        if (indexStream == null) {
